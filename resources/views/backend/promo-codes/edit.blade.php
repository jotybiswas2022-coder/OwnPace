...
@section('title', 'Edit Promo Code — '.storeName().' Admin')
@section('page_title', 'Edit Promo Code')

@section('breadcrumbs')
    @include('backend.partials.breadcrumbs', ['crumbs' => [['label' => 'Shop', 'route' => 'admin.promo-codes.index'], ['label' => $promoCode->code]]])
@endsection
...
<div class="mx-auto max-w-3xl">
    <div class="os-card overflow-hidden">
        <div class="border-b border-ink/10 px-6 py-4">
            <h3 class="font-display text-sm font-bold text-ink">Edit: <span class="font-mono">{{ $promoCode->code }}</span></h3>
        </div>
        <div class="p-6">
            <form action="{{ route('admin.promo-codes.update', $promoCode) }}" method="POST" class="space-y-5">
                @csrf
                <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    <div>
                        <label for="code" class="os-label">Code <span class="text-ember">*</span></label>
                        <input type="text" id="code" name="code" class="os-input w-full font-mono uppercase" value="{{ $promoCode->code }}" required>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" id="value" name="value" class="os-input w-full" value="{{ $promoCode->value }}" required>
                    </div>
                    <div>
                        <label for="min_order_amount" class="os-label">Min Order Amount</label>
                        <input type="number" step="0.01" id="min_order_amount" name="min_order_amount" class="os-input w-full" value="{{ $promoCode->min_order_amount }}">
                    </div>
                    <div>
                        <label for="max_uses" class="os-label">Max Uses</label>
                        <input type="number" id="max_uses" name="max_uses" class="os-input w-full" value="{{ $promoCode->max_uses }}">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="starts_at" name="starts_at" class="os-input w-full" value="{{ $promoCode->starts_at?->format('Y-m-d\TH:i') }}">
                    </div>
                    <div>
                        <label for="expires_at" class="os-label">Expiry Date</label>
                        <input type="datetime-local" id="expires_at" name="expires_at" class="os-input w-full" value="{{ $promoCode->expires_at?->format('Y-m-d\TH:i') }}">
                    </div>
                </div>
                <div class="flex items-center gap-3 border-t border-ink/10 pt-5">
                    <button type="submit" class="os-btn os-btn-brand"><i class="bi bi-check-lg"></i> Update Promo Code</button>
