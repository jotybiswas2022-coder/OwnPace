@extends('backend.app')
@section('title', 'Exchange Requests — OwnPace Admin')
    ...
    <table class="fp-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td data-label="Current Product">{{ $r->current_product?->name ?? 'N/A' }}</td>
                <td data-label="Wishlist Item">{{ $r->wishlist_product?->name ?? 'N/A' }}</td>
            ...
            @empty
            <tr class="empty-row"><td colspan="6" class="text-center py-4" style="color:var(--text-dim);">No exchange requests</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
